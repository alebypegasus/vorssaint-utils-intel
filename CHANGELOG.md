...
### Summary
Vorssaint Intel 3.5.0 brings major architecture enhancements for Intel Macs & Hackintoshes, combined with the latest upstream refinements: dedicated Intel SMC temperature and power telemetry, dynamic Extra Brightness / XDR boost on external HDR displays, text snippets visual date/time builder, radial menu profiles & favicon fetcher, and extensive stability and performance fixes.
...
- **Visual Date/Time Snippet Builder**: Popover with live previews to format dates, times and timezones in text snippets.
- **Radial Menu Profiles & Favicons**: Support for multiple custom wheel layouts, themes, triggers, and on-demand favicon downloading.
- **Liquid Glass Visuals**: Optional translucent glass visual effect toggle on macOS 26+.
- **Video & GIF Size Target**: Compress videos and GIFs directly to a target file size in megabytes.

### Fixed & Refined
- Fan curve point editing and active cooling speed adjustments are now atomic without false verification timeouts.
- Preserved rectangular aspect ratio for the menu bar battery icon.
- Prevented `Esc` from closing the clipboard preview pane prematurely.
- System monitor telemetry bounded waits preventing UI micro-stutters.
- Shell and sudoers closed-lid rules support SSO short-names and permissions by UID.
- Full uninstaller script cleanup for Intel and legacy bundle states.
