...
if (( DEV )); then
    APP_NAME="Vorssaint Utils (Developer)"
...
if (( TEST )); then
    ...
    swiftc -Onone -target "$TARGET" -sdk "$SDK" "${SDK_COMPAT_FLAGS[@]}" \
        ...
        Sources/Vorssaint/Core/CleanerExtendedStrings.swift \
        Sources/Vorssaint/Core/TenFeaturesExtendedStrings.swift \
...
if (( DEV )); then
    ...
    /usr/libexec/PlistBuddy -c "Set :CFBundleName Vorssaint Utils (Developer)" "$STAGE/Contents/Info.plist"
    /usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName Vorssaint Utils (Developer)" "$STAGE/Contents/Info.plist"
    ...
    /usr/libexec/PlistBuddy -c "Delete :MachServices:com.vorssaint.utils.fan-control" "$FAN_PLIST" 2>/dev/null || /usr/libexec/PlistBuddy -c "Delete :MachServices:com.vorssaint.utils.intel.fan-control" "$FAN_PLIST" 2>/dev/null || true
